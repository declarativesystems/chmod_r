...
define chmod_r(
  ...
  exec { "chmod -R for ${dir} (files)":
    command => "find ${dir} ${_skip} -type f ${predicate} -perm ${want_mode} -exec chmod ${want_mode} {} \\;",
  ...
  exec { "chmod -R for ${dir} (dirs)":
    command => "find ${dir} ${_skip} -type d ${predicate} -perm ${_want_mode} -exec chmod ${_want_mode} {} \\;",
